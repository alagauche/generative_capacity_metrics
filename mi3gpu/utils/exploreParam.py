...
def alphatext(ax, x, y, ls, **kw):
    ...
    if kw.get("rotation", "horizontal") == "horizontal":
        ...
        for s, c in ls:
            text = ax.text(
                x, y, s, alpha=c, transform=t, va="bottom", ha="center", **kw
            )
            text.draw(ax.figure.canvas.get_renderer())
            ex = text.get_window_extent()
...
def main():
    ...
    parser.add_argument(
        "-score", default="fbwsqrt", choices=["fb", "fbw", "fbwsqrt", "DI", "MI", "Xij"]
    )
    parser.add_argument(
        "-gauge", default="skip", choices=["skip", "nofield", "0", "w", "wsqrt"]
    )
    ...
    if args.annotimg:
        ...
        ax = plt.axes(
            (0.01 * xscale, 0.02 * yscale, 0.89 * xscale, 0.08 * yscale), sharex=main_ax
        )
        ax.set_axis_off()
        ax.imshow(
            ssim, extent=(-0.5, L - 0.5, 0, 16), interpolation="bicubic", aspect="auto"
        )
        ax = plt.axes(
            (0.9 * xscale, 0.1 * yscale, 0.08 * xscale, 0.89 * yscale), sharey=main_ax
        )
        ax.set_axis_off()
        ax.imshow(
            ssim.transpose((1, 0, 2))[::-1, :, :],
    ...
    if args.contactfreq and args.contactmode == "overlay":
        ...
        main_ax.imshow(
            cont, origin="lower", extent=(+o, L + o, +o, L + o), interpolation="nearest"
        )
    ...
    ijpicker = PositionPicker(
        contactfig, (margax, Cax, Jax), alphamap_color, J, h, ff, pottsScore
    )
