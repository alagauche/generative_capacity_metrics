...
events = [
    (l[1], int(l[2]) * units, int(l[3]) * units)
    for l in lines
    if len(l) > 0 and l[0] == "EVT"
]
names = sorted(set([name for name, start, end in events]))
t0 = events[0][1]

tot_times = [
    sum([end - start for name, start, end in events if name == n]) for n in names
]
...
plt.legend(
    [mlines.Line2D([], [], color=colors[name], label=name) for name in names], names
)
plt.show()
