...
class Vis:
    ...
    def __init__(self, config_file=None, args=None):
        ...
        self.skip = None
        self.compute_homs_script = None
        self.bvms_script = None
        self.covars_script = None
        self.data_home = None
        self.synth_nat = None
        self.which_size = None
        self.protein = None
        self.L = None
        self.A = None
        self.alpha = None
        ...
        self.tick_width = None
        self.stats_sf = None
        self.dpi = None
        self.make_hams_dist = None
        self.keep_hams = None
        self.pss = None
        self.compute_homs = None
        self.parse_homs = None
        self.plot_homs = None
        self.r20_folder = None
        self.r20_start = None
        self.r20_end = None
        self.r20_mod = None
        self.r20_data_black = None
        self.r20_data = None
        self.compute_all_covars = None
        self.which_covars = None
        self.covars = None
        self.compute_all_bvms = None
        self.which_bvms = None
        self.bvms = None
        self.keep_covars = None
        self.valseqs = None
        self.fields_couplings = None
        self.ALPHA = None 
        self.energies_file = None
    ...
    def get_bvms(self, label, msa_file):
        ...
        bvms_command = "python " + self.bvms_script + " " + label + " " \
            + msa_file + " " + source + " " + dest + " " \
            + str(self.A) + " " + str(self.keep_covars)
        
        print("\t\t\tgetting bvms for:\t", label)
        print(bvms_command)
        try:
    ...
    def get_covars(self, label, bvms_file):
        ...
        covars_command = "python " + self.covars_script + " " + label + " " \
            + bvms_file + " " + path
        
        print("\t\t\tgetting covars for:\t", label)
        print(covars_command)
        try:
            output = subprocess.check_output(['bash','-c', covars_command])
        except subprocess.CalledProcessError as e:
            raise RuntimeError("command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))

        print("completed covars for:\t" + self.covars[label])
    ...
    def plot_covars(self):
        ...
        deepsequence_covars = np.load(self.output_dir + "/" + self.covars["deepSeqs"]).ravel()
        data_home = ""
        other_covars = {"Indep" : indep_covars, "Mi3" : mi3_covars, "sVAE" : svae_covars, "DeepSeq" : deepsequence_covars}
        for label, covars in other_covars.items():
            ...
            label_text = label
            label_text = label + ", " + r"$\rho$ = " + str(round(pearson_r, self.stats_sf))     # orig with rho
            #plt.plot(target_masked, covars, 'o', markersize=marker_size, color=c, alpha=self.alpha, label=label_text)
            ax.plot(target_masked, covars, 'o', markersize=marker_size, color=c, label=label_text, zorder=self.z_order[label], alpha=0.9)
        ...
        pylab.tight_layout()
        file_name = "covars_" + self.name + "_" + str(self.keep_covars) + "_" + self.synth_nat + "_" + self.which_size + ".pdf"
    ...
    def make_homs(self):
        ...
        if self.r20_mod:
            ...
            if self.parse_homs:
                ...
                VHOMSH.parse_homs(self.r20_folder, self.output_dir, int(self.r20_start), int(self.r20_end))
    ...
    def compute_r20(self):
        ...
        #progen   = self.msa_dir + "/" + self.vis_seqs["progenSeqs"]
        ref_trunc       = self.msa_dir + "/" + self.vis_seqs["ref-trunc"]
        target_trunc    = self.msa_dir + "/" + self.vis_seqs["target-trunc"]
        
        d = " "
        '''
        homs_command = "python " + self.compute_homs_script + d + str(self.pss) \
            + d + target + d + ref + d + mi3 + d + vae + d + indep \
            + d + ref_trunc + d + target_trunc + d + self.parent_dir_name \
            + d + str(self.r20_start) + d + str(self.r20_end) + d + self.synth_nat
        '''
        homs_command = "python " + self.compute_homs_script + d + str(self.pss) \
            + d + target + d + ref + d + mi3 + d + vae + d + indep \
            + d + ref_trunc + d + target_trunc + d + deep + d + self.msa_dir \
            + d + str(self.r20_start) + d + str(self.r20_end) + d + self.synth_nat \
            + d + self.output_dir
        print("\t\t\tissuing homs_command: ", homs_command)
        try:
            output = subprocess.check_output(['bash','-c', homs_command])
        except subprocess.CalledProcessError as e:
            raise RuntimeError("command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))
    ...
    def plot_r20_mod(self):
        ...
        for file_name, data_label in datasets.items():
            ...
            if "Target" in data_label:
                if "nat" in self.synth_nat:
                    ...
                    target_data = r20_data[:,index[data_label]][0:self.r20_end-plot_limit[self.synth_nat]]
                ax.plot(list(range(self.r20_start,self.r20_end)), target_data, zorder=zorders[data_label], color=self.color_set[data_label], linestyle="dotted", linewidth=self.line_width, label=target_label, marker=black_marker, ms=ms)
                ax.errorbar(list(range(self.r20_start,self.r20_end)), target_data, zorder=zorders[data_label], color=self.color_set[data_label], linestyle="dotted", linewidth=self.line_width, label=target_label, marker=black_marker, ms=ms)
                ax2.plot(list(range(self.r20_start,self.r20_end)), target_data, zorder=zorders[data_label], color=self.color_set[data_label], linestyle="dotted", linewidth=self.line_width, label=target_label, marker=black_marker, ms=ms-1)
            elif data_label == "Mi3":
                ax.plot(list(range(self.r20_start,self.r20_end)), r20_data[:,index[data_label]][0:self.r20_end-plot_limit[self.synth_nat]], zorder=zorders[data_label], color=self.color_set[data_label], linewidth=self.line_width, label=data_label, marker="o", ms=ms)
                ax2.plot(list(range(self.r20_start,self.r20_end)), r20_data[:,index[data_label]][0:self.r20_end-plot_limit[self.synth_nat]], zorder=zorders[data_label], color=self.color_set[data_label], linewidth=self.line_width, label=data_label, marker="o", ms=ms-1)
        ...
        ax.tick_params(direction="in", axis='both', which='major', labelsize=self.tick_size, length=self.tick_length, width=self.tick_width)
        ax.set_xticks(np.arange(2, 11, 1))
        ax.set_yticks(np.arange(0, 1.2, 0.2))
        ax.set_ylim(0,1.05)
        ax2.set_xlim(1.98,2.02)
        ax2.set_ylim(0.92, 1.005)
        ax2.tick_params(direction="in", axis='both', which='major', labelsize=self.tick_size-3, length=self.tick_length-1, width=self.tick_width-0.1)
        ...
        pylab.savefig(self.data_home + "/" + self.parent_dir_name + "/" + file_name, dpi=self.dpi, format='pdf')
    ...
    def plot_r20_new(self):
        ...
        for data_label, index in datasets.items():
            ...
            if data_label == "Target":
                ...
                ax.plot(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], linestyle="dotted", linewidth=self.line_width, label=target_label, marker=black_marker, ms=ms)
                ax2.plot(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], linestyle="dotted", linewidth=self.line_width, label=target_label, marker=black_marker, ms=ms)
            else:
                ...
                ax.plot(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], linewidth=self.line_width, marker="o", ms=ms, label=data_label)
                ax2.plot(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], linewidth=self.line_width, marker="o", ms=ms, label=data_label)
        ...
        ax2.set_xticks(np.arange(2, 3, 1))
        ax2.set_yticks(np.arange(0.92, 1.02, 0.02))
        pylab.legend(fontsize=self.tick_size-3, loc=1, bbox_to_anchor=(6.5,2.5), frameon=False)
        pylab.tight_layout()
        file_name = "r20_" + self.name + "_" + str(self.pss) + "_" + self.synth_nat + "_" + self.which_size + ".pdf"
    ...
    def plot_r20(self):
        ...
        os.chdir(new_cwd)
        fig, ax = pylab.subplots(figsize=(self.fig_size,self.fig_size))
        datasets = {"mi3" : "Mi3", "vae" : "vVAE", "target" : "Target", "indep" : "Indep", "deep" : "DeepSeq"}
        n = np.arange(int(self.r20_start), int(self.r20_end))
        i = 0
        ax2 = plt.axes([0,0,1,1])
        ip = InsetPosition(ax, [0.19,0.12,0.12,0.35])
        ...
        for file_name, data_label in datasets.items():
            ...
            if data_label == "Target":
                if "nat" in self.synth_nat:
                    ...
                    target_label = "Synth-Target"
                #ax.errorbar(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], yerr=[m-lo, hi-m], linestyle="dotted", linewidth=self.line_width, fmt='.-', label=target_label, marker=black_marker, ms=ms-5)
                ax.errorbar(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], yerr=[m-lo, hi-m], linestyle="dotted", linewidth=self.line_width, fmt='.-', label=target_label, marker=black_marker, ms=ms, capsize=2, elinewidth=0.5, alpha=0.7)
                ax2.errorbar(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], yerr=[m-lo, hi-m], linestyle="dotted", linewidth=self.line_width, fmt='.-', label=target_label, marker=black_marker, ms=ms)[-1][0].set_linewidth(0)
            else:
                #ax.errorbar(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], yerr=[m-lo, hi-m], linewidth=self.line_width, fmt='.-', label=data_label, capthick=0.4, ms=ms)
                ax.errorbar(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], yerr=[m-lo, hi-m], linewidth=self.line_width, fmt='.-', label=data_label, capthick=0.4, ms=ms, capsize=2, elinewidth=0.5, alpha=0.8)
                ax2.errorbar(n, m, zorder=self.z_order[data_label], color=self.color_set[data_label], yerr=[m-lo, hi-m], linewidth=self.line_width, fmt='.-', label=data_label, capthick=0.4, ms=ms)[-1][0].set_linewidth(0)
                #pylab.errorbar(n, m, color=self.color_set[data_label], yerr=[m-lo, hi-m], fmt='.-', label=data_label, capthick=0.4, zorder=z_orders[data_label])
        ...
        ax2.set_xticks(np.arange(2, 3, 1))
        ax2.set_yticks(np.arange(0.92, 1.02, 0.02))
        pylab.legend(fontsize=self.label_size-5, loc=1, bbox_to_anchor=(6.5,2.5), frameon=False)
        pylab.tight_layout()
        file_name = "r20_" + self.name + "_" + str(self.pss) + "_" + self.synth_nat + "_" + self.which_size + ".pdf"
        pylab.savefig(self.output_dir + "/" + file_name, dpi=self.dpi, format='pdf')
        os.chdir(cwd)
    ...
    def plot_energies(self, target_energies, gpsm_energies, label):
        ...
        text = self.which_size + " " + label + r", $\rho$ = " + str(round(pearson_r, self.stats_sf))
        print(text)
        pylab.xlabel("Synth-Target Energy", fontsize=self.label_size)
        pylab.ylabel(self.which_size + " " + label + " Energy", fontsize=self.label_size)
        file_name = "loss-energy_target-" + label + "_" + self.name + ".pdf"

        # plot the data
        ax.plot(target_energies, gpsm_energies, '.', color=c, alpha=0.3, label=text, zorder=-10)
